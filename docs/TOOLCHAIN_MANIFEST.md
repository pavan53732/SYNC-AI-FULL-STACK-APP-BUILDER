...
> **NOTE**: The `sha256` values in the example above are **build-time placeholders**. Real SHA-256 hashes are computed and injected by the installer build pipeline at release time. Do not copy these values — they will not match any real binary.
