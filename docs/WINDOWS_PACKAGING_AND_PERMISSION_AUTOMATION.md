...
### 4.2 Certificate Policy (Mandatory)
    ...
    **Version increments are triggered exclusively by Kernel-confirmed mutation events (not AI proposals, not preview builds).**

    This closes the authority loop between:

    | Authority Domain | Responsibility |
    |-----------------|----------------|
    | **Mutation Authority** | Runtime Safety Kernel confirms and applies all mutations |
    | **Version Authority** | Version increments only after Kernel confirmation |
    | **Packaging Integrity** | Package version reflects only Kernel-confirmed changes |

    ##### Explicit Exclusions

    The following do NOT trigger version increments:
    - **AI Proposals**: Suggestions generated by AI Construction Engine before Kernel review
    - **Preview Builds**: Temporary builds for user preview/feedback
    - **Draft Changes**: Uncommitted work-in-progress modifications
    - **Rollback Operations**: Reverting to previous state (version unchanged)

    Only when the Runtime Safety Kernel confirms a mutation has been successfully applied does the version increment.
